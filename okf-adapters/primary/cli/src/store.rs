//! Additional traits the CLI adapter needs beyond `okf-ports`.
//!
//! These are defined here (not in `okf-ports`) because they are CLI-specific:
...
//!
//! The composition root (`okf-cli`) is responsible for implementing `CliStore`
...
///
/// Implemented by the composition root (`okf-cli`), which wires the concrete
