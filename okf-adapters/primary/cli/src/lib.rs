...
//! is `okf-cli`, the composition root, rule 6). It exposes:
//!
//! - `Cli` — the full clap argument tree (import and call `Cli::parse()` in
//!   `okf-cli/src/main.rs` to obtain parsed arguments).
...
//!
//! Architecture: imports `okf-ports` for `Clock` (and value-type re-exports).
...
/// `clock`  — UTC timestamp source (`Clock` from `okf-ports`).
///
