...
//! these `[dependencies]`. External crates (uuid) are permitted; internal okf-* adapter
//! crates are not.
...
mod tests {
    ...
    fn legacy_stubs_compile() {
        ...
        struct MinIdx;
        impl okf_ports::SearchIndex for MinIdx { fn search(&self, _: &str) -> Vec<Concept> { vec![] } }
