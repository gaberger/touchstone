...
/// Workspace root — this crate's manifest dir is `<root>/okf-conformance`.
...
pub fn touchstone_bin() -> PathBuf {
    ...
    panic!(
        "no touchstone binary found under {}/target/{{release,debug}}.\n\
         ...
         or point at another implementation:  TOUCHSTONE_BIN=/path/to/impl cargo test -p okf-conformance",
        root.display()
    );
}
