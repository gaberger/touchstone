...
//!
//! Wires `touchstone-cli-adapter` (CLI surface) with the concrete adapters:
...
use clap::Parser;
use touchstone_cli_adapter::store::{
    BundleStats, CliSearchFilter, CliStore, IndexRecord, SearchHit,
};
use touchstone_cli_adapter::{walk::find_bundle, Cli};
...
fn _touch_adapters() {
    ...
    let _ = touchstone_yaml_serde::YamlSerde;
    let _: Option<touchstone_sqlite_index::SqliteIndex> = None;
    let _: Option<touchstone_git_attest::GitAttest> = None;
    let _: Option<touchstone_crdt_sync::CrdtSync> = None;
    let _: Option<touchstone_embed_local::EmbedLocal> = None;
    let _: Option<touchstone_mcp_adapter::Surface> = None;
}
...
fn main() {
    ...
    let exit_code = touchstone_cli_adapter::run(&cli, &bundle, &mut store, &clock);
